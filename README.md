...
|type|no|enum["string", "date", "number", "boolean", "html"]|`"string"`|If `"date"`, the portal will attempt to parse the attribute value to a date; If `"Number"`, the portal will attempt to parse the attribute value to with thousand seperator; If â€œbooleanâ€, the portal will attempt to parse the attribute value to boolean value.|`"date"`|
